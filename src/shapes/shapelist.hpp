...
public:
        ...
        : debug_material(force_material)
    {
        ...
        for (int i = 0; i < num; ++i)
        {
            ...
            {
            case make_id('B','O','X'):
                memcpy(&boxes[box_count++], objects[i], sizeof(Box));
                break;
            case make_id('S','P','H'):
				memcpy(&spheres[sphere_count++], objects[i], sizeof(Sphere));
                break;
            case make_id('T','R','I'):
				memcpy(&triangles[triangle_count++], objects[i], sizeof(Triangle));
                break;
            case make_id('L','I','S','T'):
				memcpy(&lists[list_count++], objects[i], sizeof(ShapeList));
                break;
            default:
                break;
            }
