...
template<>           inline __device__ __host__ float min<float>(const float& a, const float& b)     { return fmin(a, b); }
template<>           inline __device__ __host__ float max<float>(const float& a, const float& b)     { return fmax(a, b); }
template<>           inline __device__ __host__ float3 min<float3>(const float3& a, const float3& b) { return make_float3(fmin(a.x, b.x), fmin(a.y, b.y), fmin(a.z, b.z)); }
template<>           inline __device__ __host__ float3 max<float3>(const float3& a, const float3& b) { return make_float3(fmax(a.x, b.x), fmax(a.y, b.y), fmax(a.z, b.z)); }
...
inline __device__ __host__ float3& operator*=(float3& a, const float3& b)      { a.x *= b.x; a.y *= b.y; a.z *= b.z; return a; }
inline __device__ __host__ float3  operator/(float b, const float3& a)         { return make_float3(a.x / b, a.y / b, a.z / b); }
...
inline __device__ __host__ float3 normalize(const float3& v)                   { float invLen = rsqrtf(dot(v, v)); return invLen * v; }
inline __device__ __host__ float3 reflect(const float3& i, const float3& n)    { return i - 2.f * dot(i, n) * n; }
